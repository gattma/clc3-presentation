...
### Projektstruktur
* **ParkingManagerCoordinateService** (Java, Marko): 
  * Liest Koordinaten und weitere Informationen von Parkplätzen
  * Liefert zur aktuellen Position den nähesten freien Parkplatz 
* **ParkingManagerParkingService** (Java, Marko):
  * Speichert neue Parkplätze, welche zum System hinzugefügt werden sollen
  * Liefert alle bereits gespeicherten Parkplätze
* **ParkingManagerAnalyzerService** (Python, Marko):
  * Nimmt eine Url einer Webcam entgegen und liefert die aktuelle Parkplatzsituation
...
erreicht werden.
* **maven-pipeline.yaml:** Definiert eine Openshift Pipeline, welche
...
### MongoDB - Marko
in \etc\docker\MongoDB findet sich ein Dockerfile, welches beim Hochfahren die benötigte Datenbank 
erzeugt mit mit Daten befüllt. Das hier erzeugte Image kann sowohl lokal als auch in Openshift genutzt 
werden.

Zum einfacheren Testen wurde für das Service **ParkingManagerCoordinateService** ein Mock-Service 
...
In diesem Projekt wird ein Jekins auch gleich in Openshift betrieben. Einziger Unterschied zu einem "normalen" 
Jenkins auf irgendeinem Server ist, dass in Openshift ein Jenkins-Master als Pod deployed wird und die einzelnen 
Jobs als Openshift-Pipelines angelegt werden. Dieser Ansatz ermöglicht es, den "einfachen" Jenkinsbuild um einige 
...
* Pullen der neuen Sourcen des jeweiligen Services
* Bauen des jeweiligen Services
* Ausführen der Testfälle
* Erzeugen eines neuen Dockerimages und ablegen in der Dockerregistry von Openshift
* Deployen der neuen Version
* Testen ob Deployment gut gegangen ist
* Laufende Prüfung, ob das Service noch lebt
...
* **Health:** Gibt Auskunft über das Befinden eines Pods."# CLC3-Projekt" 
